...
//!
//! Repair never follows a moving branch. It uses the trusted current commit and hands a verified
//! exact-commit rebuild to ActivationEngine's immutable replacement primitive when needed.
...
use crate::helper::{publish_installer_helper, validate_published_binary};
...
impl<D: DownloadClient, P: ProcessRunner> RepairEngine<D, P> {
    ...
    pub(crate) fn repair_with_descriptors<H: HealthChecker>(
        ...
        if let Ok(Some(runtime)) = activation.ensure_current_pointer_with_lock(&lock) {
            ...
            if let Err(error) = publish_installer_helper(&paths).and_then(|_| {
...
mod tests {
    ...
    fn prepare_installation(paths: &InstallationPaths, commit: &str) {
        let store = StateStore::new(paths.clone());
        ...
        installation.current_toolchains =
            BTreeMap::from([("jdk".to_owned(), "jdk-test".to_owned())]);
        ...
        let metadata = VersionMetadata {
            ...
            toolchains: BTreeMap::from([("jdk".to_owned(), "jdk-test".to_owned())]),
            runtime: crate::activation::RuntimeMetadata {
                desktop_executable: PathBuf::from("desktop").join(paths.desktop_executable_name()),
    ...
    #[test]
    fn direct_installation_repairs_missing_integration_surface() {
        let root = tempdir().unwrap();
        let paths = paths(root.path());
        let commit = "b".repeat(40);
        prepare_installation(&paths, &commit);
