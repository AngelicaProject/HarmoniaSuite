...
- JDK 21+
- Обновление из исходников: git + pull/сборка локально (JDK 21+ — системный или докачается сам при первом обновлении)
...
1. Забери MSI из Releases (ставится per-user, без прав админа).
2. Состояние живёт в `%APPDATA%/HarmoniaSuite`; JDK, git и unpacker уже внутри — укажи только путь к игре.
3. Обновления прилетают сами: плашка версии внизу → «Обновить сейчас». JDK, git и unpacker уже внутри — ничего докачивать не нужно.
...
MSI и портативный zip собирает CI сам на каждый тег. Обновления из программы — всегда из исходников (pull + локальная сборка).
