...
const requestedMinInstallerVersion = args.get("min-installer-version");
...
const versionContract = await readVersionContract();
if (requestedMinInstallerVersion !== undefined) {
  if (!isStableSemVer(requestedMinInstallerVersion)) {
    throw new Error(`min-installer-version must be stable SemVer: ${requestedMinInstallerVersion}`);
  }
  if (requestedMinInstallerVersion !== versionContract.minimumInstallerVersion) {
    throw new Error(
      `min-installer-version must match versions.json: ${versionContract.minimumInstallerVersion}`,
    );
  }
}
const minInstallerVersion = versionContract.minimumInstallerVersion;
...
common.minInstallerVersion = minInstallerVersion;
