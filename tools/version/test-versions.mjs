...
async function fixture({ product = "1.0.12-SNAPSHOT", installer = "0.1.2", minimum = "0.1.2" } = {}) {
...
test("valid development contract accepts independent installer version", async () => {
  const root = await fixture({ installer: "0.2.0", minimum: "0.1.2" });
  const contract = await assertVersions(root);
  assert.equal(contract.productVersion, "1.0.12-SNAPSHOT");
  assert.equal(compareSemVer("0.1.2", "0.2.0") < 0, true);
});
...
test("minimum installer cannot exceed current installer", async () => {
  const root = await fixture({ installer: "0.1.2", minimum: "0.2.0" });
...
test("changed installer source requires a higher engine version", () => {
  assert.throws(() => validateInstallerReleaseBump("0.1.2", "0.1.2", true), /not greater/);
  assert.doesNotThrow(() => validateInstallerReleaseBump("0.2.0", "0.1.2", true));
  assert.doesNotThrow(() => validateInstallerReleaseBump("0.1.2", "0.1.2", false));
});
...
test("version setters update only their contract domain", async () => {
  ...
  await exec(process.execPath, [script, "minimum-installer", "0.1.2", "--root", root], { cwd: repo });
  const contract = await assertVersions(root);
...
test("version transactions roll back every domain after injected failures", async () => {
  ...
  const cases = [
    ...
    { kind: "minimum-installer", version: "0.1.2", failAfterReplacement: 0 },
    { kind: "minimum-installer", version: "0.1.2", failAfterReplacement: 1 },
  ];
