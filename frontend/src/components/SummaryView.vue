...
import { ENTRY_STATUS_SUMMARY } from "../domain/translationStatus";
  ...
  computed: {
    ...
    rows() {
      ...
      for (const k of Object.keys(by)) {
        if (!seen.has(k) && by[k] != null && by[k] > 0)
          out.push({
            k,
            label: k,
            dot: "mut",
            v: by[k],
            share: this.share(by[k]),
          });
      }
      return out;
    },
...
<template>
  <div class="pane-body summary-pad">
    <div v-if="summary" class="sum-wrap">
      <div class="sum-top">
        <div class="sum-head">
          <span>Готово</span><b>{{ pctText }}</b>
        </div>
        <div class="sum-big">
          {{ fmt(translated) }} <span>/ {{ fmt(entries) }}</span>
        </div>
      ...
      <div v-for="r in rows" :key="r.k" class="sum-srow">
        <i class="sum-dot" :class="r.dot"></i
        ><span class="sum-lab">{{ r.label }}</span
        ><span class="grow"></span><span class="sum-share">{{ r.share }}</span
        ><b>{{ fmt(r.v) }}</b>
      </div>
      <div class="sum-sec">Проект</div>
      <div class="sum-srow">
        <span class="sum-lab">Фраз</span><span class="grow"></span
        ><b>{{ fmt(entries) }}</b>
      </div>
      <div class="sum-srow">
        <span class="sum-lab">Файлов</span><span class="grow"></span
        ><b>{{ fmt(summary.files || 0) }}</b>
      </div>
    </div>
    <div v-else class="ft-empty">
      <svg class="icon" viewBox="0 0 24 24">
        ...
        <rect x="17" y="13" width="3" height="5" /></svg
