...
export default defineComponent({
  ...
  computed: {
    ...
    groups() {
      ...
      const items = TAG_CATALOG.filter(
        (t) =>
          (this.cat === "all" || t.k === this.cat) &&
          (!q ||
...
<template>
  <div class="pane-body" style="display: flex; flex-direction: column">
    ...
    <div class="results" style="flex: 1">
      <div class="tag-ref-note">
        Кнопка + вставляет заготовку в курсор перевода. {{ "\\<" }} — буквальный
        текст, не тег; {{ "<-->" }} — тире, {{ "<->" }} — мягкий перенос.
      </div>
      <div v-for="g in groups" :key="g.kind">
        ...
        <div v-for="t in g.items" :key="t.n" class="tag-ref">
          <div style="display: flex; align-items: center; gap: 8px">
            <span :class="'tag-chip tk-' + t.k">{{ t.n }}</span
            ><span class="grow"></span
            ><button
              v-if="t.ins"
              class="ghost icon-btn sm"
              @click="insertSkeleton(t)"
              :title="insertTitle(t)"
            >
          ...
          <div
            v-if="t.k === 'fmt' && fmtSample(t.n)"
            class="color-preview"
            v-html="fmtSample(t.n)"
          ></div>
          <div
            v-if="isUiTag(t)"
            style="
          ...
          >
            ...
            >
              <input
                :value="uiId[t.n]"
                @input="uiId[t.n] = ($event.target as HTMLInputElement).value"
                placeholder="id строки UIColor"
                title="id строки UIColor"
                style="width: 90px"
              />
              <span
                class="tag-chip pal-toggle"
                @click="togglePalette(t.n)"
                :style="
                  'background:' +
                  (uiColor(t.n) || 'transparent') +
                  ';min-width:22px;min-height:22px;border:1px solid var(--border);cursor:pointer'
                "
                :title="
                  'Палитра UIColor, сейчас ' + ((uiId[t.n] || '').trim() || '—')
                "
              ></span>
              <span v-if="paletteFor === t.n" class="palette-pop">
                <button
                  v-for="c in uiEntries"
                  :key="c.id"
                  @click="pickUi(t.n, c.id)"
                  :title="c.id + ' ' + c.css"
                  :style="
                    'width:22px;height:22px;border-radius:6px;border:1px solid var(--border);background:' +
                    c.css +
                    ';cursor:pointer;padding:0;' +
                    (String(uiId[t.n]).trim() === c.id
              ...
              <button
                class="ghost sm"
                @click="$emit('insert', '<' + t.n + '(0)>')"
                title="Сброс"
              >
                0
              </button>
            </div>
            <div class="color-preview" :style="previewStyle(t.n)">
              Пример текста
            </div>
          </div>
          <div
            ...
            >
              <span
                ...
                :style="
                  ...
                  ';min-width:22px;min-height:22px;border:1px solid var(--border);cursor:pointer'
                "
                :title="'Палитра, сейчас ' + (rgb[t.n] || '—')"
              ></span>
              <input
                :value="rgb[t.n]"
                @input="rgb[t.n] = ($event.target as HTMLInputElement).value"
                placeholder="#rrggbb"
                title="Hex — точный цвет"
                style="width: 90px"
              />
              <span v-if="paletteFor === t.n" class="palette-pop">
                <button
                  v-for="c in rgbPresets"
                  :key="c"
                  @click="pickRgb(t.n, c)"
                  :title="c"
                  :style="
                    ...
                    c +
                    ';cursor:pointer;padding:0;' +
                    (rgb[t.n] === c ? 'outline:2px solid var(--primary);' : '')
                  "
                ></button>
              </span>
              <span class="grow"></span>
              <button
                class="ghost sm"
                @click="insertRgbTag(t)"
                :title="'Вставить <' + t.n + '(AARRGGBB)>'"
              >
                Вставить
              </button>
            </div>
      ...
      <div v-if="!groups.length" class="ft-empty">
