...
export default defineComponent({
  ...
  data(): EditorData {
    ...
    try {
      previewH = Math.min(
        640,
        Math.max(80, Number(localStorage.getItem("hs-ed-preview-h")) || 240),
      );
    } catch (e) {}
    return {
      tabs: [],
      ...
      drafts: {},
      translation: "",
      status: String(ENTRY_STATUS.UNTRANSLATED),
      preview: false,
      jumpError: "",
      rev: 0,
      pendingOpen: null,
  ...
  computed: {
    ...
    current() {
      return this.activeTab &&
        this.activeTab.kind === "file" &&
        this.activeTab.phraseId
        ? this.byId[this.activeTab.phraseId] || null
        : null;
    },
    currentId() {
    ...
    activeFile() {
      return (
        (this.current && this.current.file) ||
        (this.activeTab &&
          this.activeTab.kind === "file" &&
          this.activeTab.file) ||
        ""
      ...
      const txt = (e: Entry | undefined) =>
        e
          ? {
              ...
              key: e.rowKey || "",
              col: e.columnName || "",
              src: String(e.source || "").slice(0, 120),
            }
          : null;
      return {
        prev: txt(this.fileEntries[i - 1]),
        next: txt(this.fileEntries[i + 1]),
      };
    },
    canNeighborPrev() {
      return !!(
    ...
    canNeighborNext() {
      ...
      if (
        ...
        return false;
      return (
        Number(c.page) + 1 < Math.ceil(Number(c.totalGroups || 0) / pageSize)
      );
    },
    highlightedSource() {
    ...
    highlightedTranslation() {
      return highlightTags(this.translation || "", []);
    },
    gamePreview() {
      return (
        renderGamePreview(this.translation || "", this.tagIssues) ||
        '<span style="color:var(--faint);font-size:13px">Перевод пуст — введите текст, здесь появится игровой предпросмотр</span>'
      );
    },
    missingKeys() {
      return this.current
        ? findMissingTags(this.current.source || "", this.translation || "")
        : [];
    },
    sourceTags() {
      const s = this.current ? this.current.source || "" : "";
      return distinctAnon(s).concat(distinctTags(s));
    },
    tagIssues() {
      return this.current
        ? validateTags(this.current.source || "", this.translation || "")
        : [];
    },
    tagWarnings() {
      return this.current
        ? warnTags(this.current.source || "", this.translation || "")
        : [];
    },
    conflictEntry() {
    ...
    highlightedConflictSource() {
      return highlightTags(
        this.conflictEntry ? this.conflictEntry.source || "" : "",
        [],
      );
    },
    conflictOursPreview() {
      const src = this.conflictEntry ? this.conflictEntry.source || "" : "";
      const ours = this.conflictEntry
        ? this.conflictEntry.translation || ""
        : "";
      return (
        renderGamePreview(ours, validateTags(src, ours)) ||
    ...
    conflictTheirsPreview() {
      const src = this.conflictEntry ? this.conflictEntry.source || "" : "";
      const th =
        this.activeTab && this.activeTab.theirs
          ? this.activeTab.theirs.translation || ""
          : "";
      return (
        renderGamePreview(th, validateTags(src, th)) ||
        '<span style="color:var(--faint);font-size:13px">Перевод пуст</span>'
      );
    },
    fieldDirty() {
      ...
      return (
        ...
        this.status !== (e.status || ENTRY_STATUS.UNTRANSLATED)
      );
    },
    tabKey() {
      return "hs-tabs:" + (this.storeKey || "default");
    },
    defaultStatus() {
      return ENTRY_STATUS.UNTRANSLATED;
    },
    statusOptions() {
      return ENTRY_STATUS_OPTIONS as Array<{ value: string; label: string }>;
    },
  },
  watch: {
    focusId(id) {
    ...
    csvOpen(r) {
      if (r && r.file) this.openCsv(r.file);
    },
    pinRequest(r) {
      if (r && r.file) this.pinFile(r.file);
    },
    storeKey() {
      this.loadTabs();
    },
  ...
  mounted() {
    this.loadTabs();
    if (this.focusId) this.openTab(this.focusId);
    document.addEventListener("click", this.closeTabMenuOutside, true);
  },
  beforeUnmount() {
  ...
  methods: {
    ...
    dirty(t: EditorTab): boolean {
      ...
      for (const id of Object.keys(this.drafts)) {
        ...
        if (
          d.translation !== (e.translation || "") ||
          d.status !== (e.status || ENTRY_STATUS.UNTRANSLATED)
        )
          return true;
      }
      return false;
    ...
    persistTabs() {
      try {
        localStorage.setItem(
          ...
          JSON.stringify(
            this.tabs
              .filter((t) => t.kind !== "conflict")
              .map((t) =>
                t.kind === "csv"
                  ? { kind: "csv", file: t.file }
                  : t.kind === "file"
                    ? {
                        kind: "file",
                        file: t.file,
                        phraseId: t.phraseId || null,
                      }
                    : t.id,
              ),
          ),
        ...
        if (Array.isArray(raw)) {
          for (const t of raw) {
            if (t && typeof t === "object" && t.kind === "csv" && t.file) {
              ...
              const known =
                (t.phraseId && this.byId[t.phraseId]) ||
              ...
              if (known)
                this.tabs.push({
                  ...
                  file: t.file,
                  phraseId:
                    t.phraseId && this.byId[t.phraseId] ? t.phraseId : null,
                });
    ...
    openConflictTab(c: DeltaConflict): void {
      ...
      if (!t) {
        t = {
          ...
          file: c.filePath || "",
          theirs: {
            translation: (c.theirs || {}).translation || "",
            status: (c.theirs || {}).status || "",
          },
        };
        this.tabs.push(t);
        this.trimTabs(t);
    ...
    async nextUntranslated() {
      ...
      if (this.activeFile && this.rowNext) {
        try {
          const found = await this.rowNext(
            this.activeFile,
            this.current.rowIndex,
            this.current.columnIndex,
            (this.rowContext && this.rowContext.q) || "",
          );
          if (found) {
            await this.$nextTick();
            this.select(found);
    ...
    insertTag(t: string | { open: string; close?: string }): void {
      ...
      const s = ta.selectionStart ?? this.translation.length,
        e = ta.selectionEnd ?? this.translation.length;
      this.translation =
        this.translation.slice(0, s) +
        open +
        this.translation.slice(s, e) +
        close +
        this.translation.slice(e);
      this.$nextTick(() => {
        ta.focus();
        const p = s + open.length + (e - s) + (s === e ? 0 : close.length);
    ...
    async openNeighbor(delta: number): Promise<void> {
      ...
      try {
        const loaded = await this.loadRowPage(
          this.activeFile,
          page,
          context.q || "",
        );
        const cells = ((loaded && loaded.groups) || []).flatMap(
          (g) => g.cells || [],
        );
...
<template>
  <div style="display: flex; flex-direction: column; min-height: 0; flex: 1">
    <div class="ed-tabs" v-if="tabs.length">
      <div
        v-for="t in tabs"
        :key="t.kind + ':' + (t.id || t.file)"
        class="ed-tab"
        :class="{ active: t === activeTab }"
        @click="activateTab(t)"
        @mousedown="tabMouse(t, $event)"
        @contextmenu.prevent="openTabMenu(t, $event)"
        :title="tabTitle(t)"
      >
        <span
          class="ed-tab-dot"
        ...
        ></span
        ><span class="ed-tab-label">{{ tabLabel(t) }}</span
        ><button
          class="ed-tab-x"
          @click="closeTab(t, $event)"
    ...
    <CsvPreview
      v-if="activeTab && activeTab.kind === 'csv'"
      embedded
      :file="activeTab.file"
      :data="activeTab.data"
      :loading="activeTab.loading"
      :error="activeTab.error"
      @close="closeTab(activeTab, $event)"
    />
    <div
      v-if="tabMenu"
      class="dz-menu"
    ...
    >
      <div class="dz-menu-i" @click="runTabMenu('one')">Закрыть</div>
      <div class="dz-menu-i" @click="runTabMenu('others')">
        Закрыть остальные
      </div>
      <div class="dz-menu-i" @click="runTabMenu('all')">Закрыть все</div>
    </div>
    <div
      v-else-if="activeTab && activeTab.kind === 'conflict'"
      style="
        display: flex;
    ...
    >
      ...
      >
        Конфликт дельты ·
        {{ (activeTab.file || "").split("/").pop() || activeTab.id }}
      </div>
      <div class="ed-grid">
        <div class="ed-col src">
          <div class="ed-col-label">
            <span>Оригинал</span><span class="muted">{{ activeTab.id }}</span>
          </div>
        ...
        >
          <div>
            <div class="ed-col-label">
              <span
                >Наше ({{ conflictEntry ? conflictEntry.status : "—" }})</span
              >
            </div>
            <div class="ed-text" v-html="conflictOursPreview"></div>
          </div>
          <div>
            <div class="ed-col-label">
      ...
      <div style="display: flex; gap: 8px; flex-wrap: wrap">
        ...
        ><button
          class="ghost sm"
          @click="$emit('resolve', { id: activeTab.id, mode: 'ours' })"
        >
          Оставить наше
        </button>
      </div>
    </div>
    <div v-else-if="!current" class="ft-empty" style="flex: 1">
      <svg class="icon" viewBox="0 0 24 24">
        <path d="M12 20h9" />
        <path d="M16.5 3.5a2.1 2.1 0 0 1 3 3L7 19l-4 1 1-4 12.5-12.5z" /></svg
      ><span>Выберите файл — он откроется новой вкладкой</span>
    </div>
    <template v-else>
      <div class="editor-head">
        ...
        <span v-if="jumpError" style="color: var(--danger); font-size: 12px">{{
          jumpError
        }}</span>
        <span class="grow"></span>
        <button
          class="ghost ed-follow-row"
          :class="{ active: followRow }"
          :aria-pressed="followRow"
          @click="$emit('toggle-follow-row')"
          :title="
            followRow
              ? 'Отключить подсветку и автопереход строки'
              : 'Включить подсветку и автопереход строки'
          "
        >
          <svg class="icon" viewBox="0 0 24 24">
        ...
        <button
          class="ghost"
          @click="nextUntranslated"
          title="Следующая непереведённая"
        >
          <svg class="icon" viewBox="0 0 24 24">
      ...
      <div class="ed-where">
        <span
          class="ed-where-file"
          @click="$emit('reveal', activeFile)"
          title="Показать в файлах проекта"
          style="cursor: pointer"
          >{{ activeFile }}</span
        ><button
          class="ghost icon-btn sm"
          @click="pinFile(activeFile)"
          title="Закрепить превью таблицы под переводом"
        >
          <svg class="icon" viewBox="0 0 24 24">
        ...
        ><span class="muted">
          · строка {{ current.rowKey || (current.rowIndex || 0) + 1 }} ·
          {{ current.columnName || "кол." + current.columnIndex }}</span
        ><span class="grow"></span
        ><span v-if="filePos" class="muted"
          >{{ filePos.i }} из {{ filePos.n }} в файле</span
        >
      </div>
      <div
        v-if="
          neighbors.prev || neighbors.next || canNeighborPrev || canNeighborNext
        "
        class="ed-nbrs"
      >
        <div
          v-if="neighbors.prev"
          class="ed-nbr"
          @click="openNeighbor(-1)"
          :title="neighbors.prev.src"
        >
          ↑ {{ neighbors.prev.key }} · {{ neighbors.prev.src }}
        </div>
        <div
          v-else-if="canNeighborPrev"
          class="ed-nbr muted"
          @click="openNeighbor(-1)"
        >
          ↑ Предыдущая страница
        ...
        <div
          ...
          class="ed-nbr"
          @click="openNeighbor(1)"
          :title="neighbors.next.src"
        >
          ↓ {{ neighbors.next.key }} · {{ neighbors.next.src }}
        </div>
        <div
          v-else-if="canNeighborNext"
          class="ed-nbr muted"
          @click="openNeighbor(1)"
        >
          ↓ Следующая страница
      ...
      <div class="ed-grid">
        <div class="ed-col src">
          <div class="ed-col-label">
            <span>Оригинал</span
            ><span class="muted">{{ current ? current.id : "" }}</span>
          </div>
          <div class="ed-text" v-html="highlightedSource"></div>
          <div class="tag-chips" v-if="sourceTags.length">
            <button
              v-for="t in sourceTags"
              :key="t"
              :class="
                'tag-chip tk-' +
                tagKindOf(t) +
                (missingKeys.includes(normTag(t)) ? ' tk-missing' : '')
              "
              @click="insertTag(t)"
              :title="tagKindLabel(tagKindOf(t)) + ': ' + t + ' — вставить'"
            >
              {{ t }}
            </button>
          </div>
        ...
        <div class="ed-col">
          <div class="ed-col-label">
            <span
              >Перевод · {{ current ? translation.length : 0 }} симв.<span
                v-if="fieldDirty"
            ...
            ><span style="display: flex; gap: 4px; align-items: center"
              ...
              ><span
                class="tag-state"
                :class="{
                  bad: tagIssues.length,
                  warn: !tagIssues.length && tagWarnings.length,
                }"
                >теги:
                {{
                  tagIssues.length
          ...
          ></div>
          <div class="tag-error" v-if="tagIssues.length">
            <button
              v-for="(it, i) in tagIssues"
              :key="i"
              class="tag-err-item"
              @click="jumpToPos(it.pos)"
              :title="'Перейти к позиции ' + it.pos"
            >
              {{ it.message }}
            </button>
          </div>
          <div class="tag-warn" v-else-if="tagWarnings.length">
            {{ tagWarnings.join("; ") }}
          </div>
        </div>
      </div>
      <div class="ed-foot">
        <span
          :class="
            'ed-status-pill status-' +
            statusTone(current ? current.status : defaultStatus)
          "
          >{{ current ? current.status : "—" }}</span
        >
        <div class="st-wrap">
          <button
            type="button"
            ...
            @keydown.esc="statusOpen = false"
            :title="'Статус: ' + statusLabel(status)"
          >
            <i class="sum-dot" :class="statusDot(status)"></i
          ...
          <div v-if="statusOpen" class="dd-menu up st-menu">
            <div
              ...
              :title="o.label"
            >
              <i class="sum-dot" :class="statusDot(o.value)"></i
        ...
        <button
          class="primary"
          @click="save(true)"
          :disabled="!fieldDirty || tagIssues.length > 0"
          :title="
            tagIssues.length
              ? 'Исправьте теги перед сохранением'
              : fieldDirty
        ...
        <button @click="nextUntranslated" title="Следующая непереведённая">
          <svg class="icon" viewBox="0 0 24 24">
        ...
        <button class="ghost icon-btn sm" @click="prev()" title="Предыдущая">
          <svg class="icon" viewBox="0 0 24 24">
            <path d="M18 15l-6-6 6-6" />
          </svg>
        </button>
        <button class="ghost icon-btn sm" @click="next()" title="Следующая">
          <svg class="icon" viewBox="0 0 24 24"><path d="M6 9l6 6 6-6" /></svg>
        </button>
        <button
          class="ghost icon-btn sm"
          @click="translation = current ? current.source : ''"
    ...
    <div v-if="pinned" class="ed-pin">
      <div
        class="ed-pin-handle"
        @mousedown="startPinResize"
        title="Потяните, чтобы изменить высоту"
      ></div>
      <div class="ed-pin-head">
        <span class="ed-where-file">{{ pinned }}</span
        ><span class="grow"></span
        ><button
          class="ghost icon-btn sm"
          @click="pinFile(pinned)"
          title="Обновить"
        >
          <svg class="icon" viewBox="0 0 24 24">
      ...
      <div class="ed-pin-body" :style="'height:' + previewH + 'px'">
        <CsvPreview
          embedded
          :file="pinned"
          :data="pinData"
