...
export default defineComponent({
  ...
  watch: {
    ...
    job(nv) {
      if (
        nv &&
        nv.status !== "running" &&
        (nv.action === "merge" || nv.action === "gemini")
      )
        this.load();
    },
  },
  ...
  methods: {
    ...
    applyEntryUpdate(update: EntryUpdate): void {
      ...
      const delta =
        Number(this.isTranslated(entry)) -
        Number(this.isTranslated(update.previous));
      const next = stats
    ...
    async tryDownload(f: string): Promise<void> {
      ...
      try {
        const b = await api.downloadExportCsv(
          this.projectId,
          f.split("/").pop() || "",
        );
...
<template>
  <div class="pane-body summary-pad">
    <div
      ...
      "
    >
      Пак перевода Harmonia
    </div>
    <div class="muted" style="font-size: 12px; margin: 6px 0">
      Файлов с переводами: {{ candidates.length }} · собрано: {{ builtCount }} ·
      manifest.json: {{ manifestBuilt ? "да" : "нет" }}
    </div>
    <div class="muted" style="font-size: 12px; margin-bottom: 8px">
      Выбор для сборки:
      {{
        scope && scope.size
          ? scope.size + " (собираются только отмеченные)"
          : "все таблицы"
      }}
    </div>
    <div style="margin: 8px 0">
      <button
        class="primary grow"
        style="width: 100%"
        @click="$emit('build-download')"
        :disabled="building || !candidates.length"
      >
        <svg class="icon" viewBox="0 0 24 24">
          <path d="M21 8l-9-5-9 5 9 5 9-5z" />
          <path d="M3 13l9 5 9-5" /></svg
        >{{ building ? "Сборка…" : "Собрать пак и скачать" }}
      </button>
    </div>
    <div style="display: flex; gap: 8px; margin: 8px 0; align-items: center">
      <button
        :style="{ visibility: scope && scope.size ? 'visible' : 'hidden' }"
        class="sm"
        @click="$emit('clear-scope')"
        title="Сбросить выбор — собирать все таблицы"
      >
      ...
      ><button
        ...
        @click="$emit('build')"
        :disabled="building || !candidates.length"
        title="Собрать CSV в exported_csv без скачивания"
      >
        Только собрать
      </button>
    </div>
    ...
    >
      ZIP без манифеста Harmonia отклонит — заполните вкладку «Пак».
    </div>
    ...
    >
      <div
        ...
        :key="f.path"
        class="export-row"
        @click="$emit('toggle-scope', f.path, !inScope(f.path))"
        style="cursor: pointer"
      >
        <input
          type="checkbox"
          :checked="inScope(f.path)"
          @click.stop
          @change="
            $emit(
              'toggle-scope',
        ...
        ><span class="muted" style="font-size: 11px; white-space: nowrap"
          >{{ f.translated }}/{{ f.total }}</span
        ><span v-if="isBuilt(f.path)" class="ft-ok export-badge">собран</span
        ><span v-else class="muted export-badge">не собран</span
        ><button
          v-if="isBuilt(f.path)"
          class="ghost icon-btn sm"
          @click.stop="tryDownload(f.path)"
          title="Скачать CSV"
        >
          <svg class="icon" viewBox="0 0 24 24">
    ...
    <div v-else class="ft-empty">
      <svg class="icon" viewBox="0 0 24 24">
        ...
        <path d="M3 13l9 5 9-5" /></svg
      ><span
        >Нет файлов с переводами.<br />Переведите строки в редакторе или через
        Gemini.</span
      >
    </div>
  </div>
