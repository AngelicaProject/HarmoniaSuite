...
export default defineComponent({
  ...
  computed: {
    sections() {
      return [
        { id: "sources", title: "Источники данных" },
        { id: "ai", title: "ИИ-перевод" },
        { id: "backup", title: "База данных" },
      ];
    },
    syncRunning() {
      return (
        !!this.syncJob &&
        (this.syncJob.status === "running" || this.syncJob.status === "queued")
      );
    },
    syncTail() {
    ...
    liveEstimate() {
      ...
      const per = this.bkItems.length
        ? this.bkItems[0].size || 0
        : this.bkRetention
          ? Math.round((this.bkEstimate || 0) / this.bkRetention)
          : 0;
      return n * per;
    },
    fillPct() {
    ...
    keyPlaceholder() {
      ...
      return this.aiSt.geminiKeySource === "env"
        ? "Ключ из окружения (GEMINI_API_KEY)"
        : "Не задан";
    },
    orKeyPlaceholder() {
      if (this.aiSt.openrouterKeySet)
        return this.aiSt.openrouterKeyHint || "••••";
      return this.aiSt.openrouterKeySource === "env"
        ? "Ключ из окружения (OPENROUTER_API_KEY)"
        : "Не задан";
    },
  },
  watch: {
  ...
  methods: {
    ...
    async aiSave() {
      ...
      this.aiSaving = true;
      try {
        this.aiSt = await api.saveAiSettings({
        ...
        this.geminiKey = "";
        this.openrouterKey = "";
        if (this.aiSt.openrouterModel)
          this.openrouterModel = this.aiSt.openrouterModel;
  ...
  <div class="overlay" style="z-index: 1500">
    <div class="modal set-modal">
      ...
      <div class="set-body">
        <div class="set-nav">
          <button
            ...
            @click="section = s.id"
          >
            <svg v-if="s.id === 'sources'" class="icon" viewBox="0 0 24 24">
              <path
                d="M3 7a2 2 0 0 1 2-2h3l2 2h9a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V7z"
              /></svg
            ><svg v-else-if="s.id === 'ai'" class="icon" viewBox="0 0 24 24">
              <path
            ...
            ><svg v-else class="icon" viewBox="0 0 24 24">
              <path
                d="M4 6c0-1.7 3.6-3 8-3s8 1.3 8 3-3.6 3-8 3-8-1.3-8-3zm0 0v12c0 1.7 3.6 3 8 3s8-1.3 8-3V6"
              />
        ...
        <div class="set-content">
          <template v-if="section === 'sources'">
            <div class="set-field">
              ...
              <div class="set-row">
                ...
                /><button class="subtle" @click="detect">Найти</button>
              </div>
              <div class="set-hint">Версия: {{ srcSt.gameVersion || "—" }}</div>
            </div>
            <div class="set-hint">
              Источники: {{ srcSt.ready ? "готовы" : "не синхронизированы" }}
            </div>
            <div v-if="srcError" class="form-error">{{ srcError }}</div>
            <div v-else-if="srcSaved" class="set-ok">{{ srcSaved }}</div>
            <div class="set-actions">
              <button class="primary" @click="srcSave" :disabled="srcSaving">
                Сохранить</button
              ><button @click="runSync" :disabled="syncRunning">
                Синхронизировать
              </button>
            </div>
            <div v-if="syncJob" class="set-field">
              <div class="set-hint">
                Синхронизация:
                {{ syncRunning ? "выполняется…" : syncJob.status }}
              </div>
              <pre
                ref="syncBox"
                class="log"
                ...
                >{{ syncTail }}</pre>
              <div class="set-actions">
                <button v-if="syncRunning" class="ghost" @click="cancelSync">
                  Отмена
                </button>
              </div>
            </div>
          ...
          <template v-else-if="section === 'ai'">
            <p class="set-intro">
              Провайдер нейроперевода и ключи. Ключ из настроек важнее
              переменной окружения; пустое поле оставляет сохранённый ключ как
              есть, стереть — кнопкой «Стереть».
            </p>
            <div class="set-field">
              <span class="set-label">Провайдер</span>
              <div class="seg" style="max-width: 340px">
                <button
                  :class="{ on: provider === 'gemini' }"
                  @click="provider = 'gemini'"
                >
                  Gemini
                </button>
                <button
                  :class="{ on: provider === 'openrouter' }"
            ...
            <template v-if="provider === 'gemini'">
              <div class="set-field">
                ...
                <div class="set-row">
                  ...
                  /><button
                    class="subtle"
                    @click="checkKey('gemini')"
                    :disabled="checkBusy === 'gemini'"
                  >
                    Проверить</button
                  ><button
                    v-if="aiSt.geminiKeySet"
                    class="ghost"
                    @click="clearKey('gemini')"
                    title="Стереть сохранённый ключ"
                  >
                    Стереть
                  </button>
                </div>
                <div class="set-keyline">
                  <i
                    class="status-dot"
                    :class="
                      aiSt.geminiKeySet || aiSt.geminiKeySource === 'env'
                        ? 'on'
                        : 'off'
                    "
                  ></i
                  ><span>{{
                    aiSt.geminiKeySet
                      ? "Ключ настроен"
                      : aiSt.geminiKeySource === "env"
                        ? "Ключ из окружения (GEMINI_API_KEY)"
                        : "Ключ не задан"
                  }}</span
                  ><span
                    v-if="checkMsg.gemini"
                    :style="{
                      color: checkMsg.gemini.ok
                        ? 'var(--success)'
                        : 'var(--danger)',
                    }"
                    >· {{ checkMsg.gemini.text }}</span
                  >
                </div>
                <div class="set-hint">
                  Модель выбирается при запуске перевода
                </div>
              </div>
            </template>
            <template v-else>
              <div class="set-field">
                ...
                <div class="set-row">
                  <input
                    ...
                    type="password"
                    class="grow"
                    :placeholder="orKeyPlaceholder"
                    autocomplete="off"
                  /><button
                    class="subtle"
                    @click="checkKey('openrouter')"
                    :disabled="checkBusy === 'openrouter'"
                  >
                    Проверить</button
                  ><button
                    v-if="aiSt.openrouterKeySet"
                    class="ghost"
                ...
                <div class="set-keyline">
                  <i
                    class="status-dot"
                    :class="
                      aiSt.openrouterKeySet ||
                      aiSt.openrouterKeySource === 'env'
                        ? 'on'
                        : 'off'
                    "
                  ></i
                  ...
                  ><span
                    v-if="checkMsg.openrouter"
                    :style="{
                      color: checkMsg.openrouter.ok
                        ? 'var(--success)'
                        : 'var(--danger)',
                    }"
                    >· {{ checkMsg.openrouter.text }}</span
                  >
                </div>
              </div>
              <div class="set-field">
                ...
                />
                <div class="set-hint">
                  ID модели OpenRouter; при запуске можно указать другую.
                  Фактическая модель при автовыборе пишется в журнал перевода
                </div>
              </div>
              <div class="set-field">
                ...
                <div class="seg" style="max-width: 420px">
                  <button
                    :class="{ on: !openrouterReasoning }"
                    @click="openrouterReasoning = ''"
                  >
                    Выкл
                  </button>
                  <button
                    :class="{ on: openrouterReasoning === 'low' }"
                    @click="openrouterReasoning = 'low'"
                  >
                    Low
                  </button>
                  <button
                    :class="{ on: openrouterReasoning === 'medium' }"
                    @click="openrouterReasoning = 'medium'"
                  >
                    Medium
                  </button>
                  <button
                    :class="{ on: openrouterReasoning === 'high' }"
                    @click="openrouterReasoning = 'high'"
                  >
                    High
                  </button>
                </div>
                <div class="set-hint">
                  Передаётся только моделям с поддержкой reasoning (видно из
                  каталога); остальным параметр пропускается
                </div>
              </div>
            </template>
          ...
          <template v-else-if="section === 'backup'">
            <div class="set-field">
              ...
              <div class="set-row">
                <button class="subtle" @click="bkCreate" :disabled="bkBusy">
                  Создать резервную копию</button
                ><button class="subtle" @click="bkOpenFolder">
                  Открыть папку
                </button>
              </div>
              <div class="set-hint">
                Снимок базы целиком; хранится в каталоге backups рядом с базой
              </div>
            </div>
            <div class="set-field">
              <span class="set-label">Журнал приложения</span>
              <div class="set-row">
                <button class="subtle" @click="copyLog" :disabled="logBusy">
                  {{ logBusy ? "Копирование…" : "Скопировать журнал" }}
                </button>
              </div>
              <div class="set-hint">
                Хвост журнала нужен для диагностики ошибок обновления и других
                операций
              </div>
            ...
            <div class="set-field">
              ...
              <div class="set-hint">
                ≈ {{ fmtBytes(liveEstimate) }} при {{ bkInput }} копиях (сейчас
                занято {{ fmtBytes(bkUsed) }})
              </div>
            </div>
            <div class="set-field">
              ...
              <div class="set-row" style="align-items: center">
                <input
                  ...
                  @change="bkSaveAuto"
                /><span class="set-hint" style="margin: 0">мин, 0 — выкл</span>
              </div>
              <div class="set-hint">
                Автоматическое создание {{ fmtInterval(bkAuto) }} · создание
                копии вручную сбрасывает таймер
              </div>
            </div>
            ...
            >
              ...
              ><a
                ...
                download
                >Скачать</a
              ><button class="ghost" @click="bkDelete(b.name)">Удалить</button>
            </div>
            <div v-if="!bkItems.length && !bkError" class="set-hint">
      ...
      <div class="set-foot">
        ...
        ><button v-if="!forced" @click="$emit('close')">Отмена</button
        ><button class="ghost" @click="applyAll">Применить</button>
      </div>
    </div>
  </div>
</template>
