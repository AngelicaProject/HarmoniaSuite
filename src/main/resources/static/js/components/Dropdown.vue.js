export default {
  ...
  methods: {
    ...
    show() {
      this.open = true;
      const i = this.cur ? this.norm.findIndex(o => String(o.value) === String(this.cur.value)) : 0;
      this.hl = i >= 0 ? i : 0;
  ...
  <div class="dd" :style="width ? 'width:' + width : ''">
    <button type="button" class="dd-btn" @click="toggle" :title="title || curLabel">
      <span class="dd-label">{{curLabel}}</span><svg class="icon" viewBox="0 0 24 24"><path d="M6 9l6 6 6-6"/></svg>
    </button>
    <div v-if="open" class="dd-menu" :class="[up ? 'up' : '', menuAlign]">
      <div v-for="(o, i) in norm" :key="String(o.value)" class="dd-item" :class="{sel: cur && String(o.value) === String(cur.value), hl: i === hl}" @click="pick(o)" @mouseenter="hl = i" :title="o.label"><span class="dd-text">{{o.label}}</span></div>
      <div v-if="!norm.length" class="muted" style="padding:7px 10px;font-size:12px">Нет вариантов</div>
    </div>
  </div>`
}
