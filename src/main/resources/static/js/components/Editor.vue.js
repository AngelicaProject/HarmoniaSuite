...
export default {
  ...
  props: ['entries', 'focusId', 'storeKey', 'csvOpen', 'csvRoot', 'pinRequest', 'rowContext', 'rowNext', 'loadRowPage', 'followRow'],
  emits: ['save', 'navigate', 'need-entry', 'resolve', 'reveal', 'file', 'entry', 'toggle-follow-row'],
  data() {
    let previewH = 240;
  ...
  computed: {
    ...
    currentId() { return this.current ? this.current.id : ''; },
  ...
  watch: {
    ...
    currentId(id) { if (id) this.$emit('entry', id); },
    statusOpen(v) {
  ...
  <div style="display:flex;flex-direction:column;min-height:0;flex:1">
    ...
    <div class="editor-head">
      ...
      <button class="ghost ed-follow-row" :class="{active:followRow}" :aria-pressed="followRow" @click="$emit('toggle-follow-row')" :title="followRow?'Отключить подсветку и автопереход строки':'Включить подсветку и автопереход строки'"><svg class="icon" viewBox="0 0 24 24"><circle cx="12" cy="12" r="3"/><path d="M12 2v3M12 19v3M2 12h3M19 12h3"/></svg>Следить за строкой</button>
