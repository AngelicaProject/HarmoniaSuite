CREATE OR REPLACE FUNCTION uuid6() RETURNS uuid LANGUAGE plpgsql AS $$
DECLARE
    ts bigint := ((extract(epoch FROM clock_timestamp()) * 10000000)::bigint
        + 122192928000000000) & 1152921504606846975;
    r1 bigint := (random() * 2147483648)::bigint;
    r2 bigint := (random() * 2147483648)::bigint;
    rand62 bigint := (r1 << 31) | r2;
BEGIN
    RETURN (lpad(to_hex((ts >> 28) & 4294967295), 8, '0') || '-'
        || lpad(to_hex((ts >> 12) & 65535), 4, '0') || '-6'
        || lpad(to_hex(ts & 4095), 3, '0') || '-'
        || lpad(to_hex(((rand62 >> 48) & 16383) | 32768), 4, '0') || '-'
        || lpad(to_hex(rand62 & 281474976710655), 12, '0'))::uuid;
END;
$$;
