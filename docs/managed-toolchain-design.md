...
is explicit and never falls back to the host's system Java or Node. Git is not a
managed archive: Phase 4 will use the same Rust/libgit2 implementation on
Windows and Linux for clone, fetch, ref resolution, and worktree operations.

## Pinned descriptors

`ToolchainDescriptor` is the only input accepted by the manager. It contains the
tool kind, exact version, target, HTTPS URL, archive format, SHA-256, safe
relative `home_dir`, and executable paths. Descriptor validation rejects missing or malformed
...
contains the descriptor identity, archive digest, installation directory, safe
relative `home_dir`, and resolved executable paths. `InstallationState` keeps current and previous
...
are prepended to `PATH`, `JAVA_HOME` points at the descriptor's managed JDK
`home_dir`, and Maven and npm
...
duplicate/colliding entries, hard links, special files, and Windows reparse
points are rejected. Relative symlinks are allowed only when their normalized
target remains inside the extraction root; absolute and escaping targets are
rejected. Extraction remains inside a managed staging root;
metadata is committed only after all files and required executables exist. A
failed extraction or post-extraction verification removes only the
installer-owned staging directory. Final directories orphaned by a crash before
state commit are removed only when their generated ID is recognized. Garbage
collection first records a pending transaction, renames installations into
managed trash, commits metadata, and only then deletes trash; startup recovery
either restores the pre-commit move or completes the post-commit cleanup.
...
install or select the product JDK or Node; Git uses Rust/libgit2 in Phase 4.
