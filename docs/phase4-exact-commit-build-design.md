...
Before component builds run, the pipeline reads `versions.json` from the exact checkout. Its
`productVersion` is the canonical product identity for the result. Maven `project.version`,
frontend package version, and desktop package version must match it; any mismatch fails closed.
...
transaction pins the exact toolchain IDs before build work begins, so Phase 3 GC cannot remove
them.
