...
assets verify. Rolling publication runs only after successful `main` CI, rejects stale workflow
heads, persists SHA-to-generation mapping under `rolling/generations`, invokes the Rust production
verifier, and updates stable aliases last. A rerun for the same SHA is idempotent; a generation
